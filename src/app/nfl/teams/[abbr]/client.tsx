...
function computeTeamRecord(results: BacktestResult[], abbr: string): TeamRecord {
  ...
  for (const r of results) {
    ...
    // ATS - result is from perspective of our pick, need to determine if we picked this team
    if (r.atsResult) {
      // For simplicity, track raw results - the pick logic is complex
...
export default function NFLTeamClient() {
  const params = useParams();
  const rawAbbr = params.abbr as string;
  const abbr = rawAbbr.toUpperCase();
  const teamName = NFL_TEAMS[abbr] || abbr;
  ...
  if (notFound) {
    return (
      <div className="text-center py-16">
        ...
        <Link href="/rankings" className="text-red-600 hover:text-red-700 font-medium">
          ← View all teams
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-gray-500">
        <Link href="/rankings" className="hover:text-red-600">Rankings</Link>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-2 text-sm">
              <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${
                powerRank <= 3 ? 'bg-green-600 text-white' :
        ...
        {team && (
          <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-gray-100">
            <div>
              <div className="text-sm text-gray-500">Points Per Game</div>
              <div className="text-xl font-bold text-gray-900">{team.ppg?.toFixed(1) || '—'}</div>
            </div>
            <div>
              <div className="text-sm text-gray-500">Points Allowed</div>
              <div className="text-xl font-bold text-gray-900">{team.ppgAllowed?.toFixed(1) || '—'}</div>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-50 rounded-lg p-4 text-center">
              <div className="text-sm text-gray-500 mb-1">Against the Spread</div>
            ...
            <table className="w-full text-sm">
              <thead className="text-gray-500 border-b border-gray-200">
                <tr>
                  ...
                  <th className="pb-2 text-center font-medium">ATS</th>
