...
function computeAnalysis(results: BacktestResult[]): Analysis {
  ...
  if (bestPct > 55) {
    insights.push(`${bestSize.charAt(0).toUpperCase() + bestSize.slice(1)} puck lines perform best at ${bestPct}%.`);
  }

  return {
    summary: {
      totalGames: results.length,
      avgSpreadError: Math.round(avgSpreadError * 100) / 100,
      medianSpreadError: Math.round(medianSpreadError * 100) / 100,
      avgPredictedMargin: Math.round(avgPredictedMargin * 100) / 100,
      avgActualMargin: Math.round(avgActualMargin * 100) / 100,
    ...
    bySpreadSize: {
      small: { ...smallSpread, winPct: smallWinPct, range: '<0.5 goals' },
      medium: { ...mediumSpread, winPct: mediumWinPct, range: '0.5-1.5 goals' },
      large: { ...largeSpread, winPct: largeWinPct, range: '1.5+ goals' },
    },
...
export default function NHLResultsPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const blobRes = await fetch('/nhl-prediction-data.json', { cache: 'no-cache' });
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              ...
              <div className="text-2xl font-bold text-gray-900">{analysis.summary.avgSpreadError} goals</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="text-gray-500 text-xs font-medium mb-1">Median Error</div>
              <div className="text-2xl font-bold text-gray-900">{analysis.summary.medianSpreadError} goals</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-3">By Predicted Margin</h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
          ...
          {/* Biggest Misses */}
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <h3 className="font-bold text-gray-900 mb-3">Biggest Puck Line Misses</h3>
          ...
          <table className="w-full text-xs sm:text-sm min-w-[600px]">
            <thead className="bg-gray-50 text-gray-600 border-b border-gray-200">
              <tr>
                ...
                <th className="px-2 sm:px-4 py-2 sm:py-3 text-center font-semibold">PL</th>
