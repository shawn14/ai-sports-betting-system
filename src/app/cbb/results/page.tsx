...
function computeAnalysis(results: BacktestResult[]): Analysis {
  ...
  for (const r of results) {
    ...
    // CBB spread sizes (different thresholds than NFL)
...
export default function CBBResultsPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const blobRes = await fetch('/cbb-prediction-data.json', { cache: 'no-cache' });
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-200 pb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">CBB Results</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-1.5 sm:py-2 bg-purple-500 hover:bg-purple-600 text-white text-xs sm:text-sm font-medium rounded-lg transition-colors"
          >
