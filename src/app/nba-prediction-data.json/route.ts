...
export async function GET() {
  try {
    ...
    let blobMetadata;
    try {
      blobMetadata = await head('nba-prediction-data.json');
    } catch {
      ...
      return NextResponse.json({
        ...
        error: 'Data not synced yet. Run /api/cron/nba-sync to generate data.',
      }, {
        headers: {
    ...
    console.error('Error fetching NBA blob data:', error);
    return NextResponse.json({
      error: 'Failed to load NBA prediction data',
