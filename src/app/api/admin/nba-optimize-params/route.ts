...
export async function GET() {
  try {
    // Fetch NBA backtest data from blob
    ...
    if (gamesWithVegas.length < 10) {
      return NextResponse.json({
        ...
        message: 'Run /api/admin/nba-fetch-historical-odds first to backfill Vegas odds',
      }, { status: 400 });
    }

    // Current NBA parameters
    const currentParams = {
    ...
    // Grid search ranges for NBA (adjusted for higher-scoring games)
    const spreadRegressionRange = [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7];
    const eloToPointsRange = [0.03, 0.04, 0.05, 0.06, 0.07, 0.08, 0.1];
    const homeCourtAdvRange = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0];
    const eloCapRange = [0, 10, 15, 20, 25, 30];
    ...
    const focusedSR = [0.4, 0.45, 0.5, 0.55, 0.6];
    const focusedETP = [0.04, 0.05, 0.06, 0.07, 0.08];
    const focusedHCA = [2.0, 2.5, 3.0, 3.5];
    const focusedEC = [15, 20, 25];
