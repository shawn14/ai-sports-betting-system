...
export async function fetchCollegeBasketballOdds(): Promise<Map<string, Partial<Odds>[]>> {
  const apiKey = process.env.NEXT_PUBLIC_ODDS_API_KEY;
  if (!apiKey) throw new Error('Odds API key not configured');

  // Add timestamp to bust any caching
  const timestamp = Date.now();
  const url = `${ODDS_API_BASE}/sports/basketball_ncaab/odds/?apiKey=${apiKey}&regions=us&markets=spreads,totals,h2h&oddsFormat=american&_t=${timestamp}`;
