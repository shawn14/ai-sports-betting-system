...
export async function GET() {
  try {
    ...
    let blobMetadata;
    try {
      blobMetadata = await head('nhl-prediction-data.json');
    } catch {
      ...
      return NextResponse.json({
        ...
        error: 'Data not synced yet. Run /api/cron/nhl-sync to generate data.',
      }, {
        headers: {
    ...
    console.error('Error fetching NHL blob data:', error);
    return NextResponse.json({
      error: 'Failed to load NHL prediction data',
