...
export async function GET() {
  try {
    ...
    let blobMetadata;
    try {
      blobMetadata = await head('cbb-prediction-data.json');
    } catch {
      ...
      return NextResponse.json({
        ...
        error: 'Data not synced yet. Run /api/cron/cbb-sync to generate data.',
      }, {
        headers: {
    ...
    console.error('Error fetching CBB blob data:', error);
    return NextResponse.json({
      error: 'Failed to load CBB prediction data',
