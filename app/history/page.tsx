...
import { BarChart3, TrendingUp, Trophy, Database } from 'lucide-react';
...
export default function HistoryPage() {
  ...
  const getOurPick = (game: GameResult) => {
    // We bet on the home team (negative spread means home favored)
    const [away, home] = game.matchup.split(' @ ');
    const homeAbbr = home.split(' ').pop() || home;
    const awayAbbr = away.split(' ').pop() || away;

    if (game.predicted_spread < 0) {
      return `${homeAbbr} ${formatSpread(game.predicted_spread)}`;
    } else {
      return `${awayAbbr} ${formatSpread(-game.predicted_spread)}`;
    }
  };
  ...
  const pushes = gameHistory.filter(g => g.result === 'PUSH').length;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-6">
              <div className="text-center">
                <div className="text-green-400 text-3xl font-bold">{wins}</div>
                <div className="text-slate-400 text-xs">Wins</div>
              </div>
              <div className="text-slate-600 text-2xl">-</div>
              <div className="text-center">
                <div className="text-red-400 text-3xl font-bold">{losses}</div>
                <div className="text-slate-400 text-xs">Losses</div>
              </div>
              <div className="text-slate-600 text-2xl">-</div>
              <div className="text-center">
                <div className="text-yellow-400 text-3xl font-bold">{pushes}</div>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  >
                    ...
                    <option value="WIN">Wins Only ({wins})</option>
                    <option value="LOSS">Losses Only ({losses})</option>
                    <option value="PUSH">Pushes Only ({pushes})</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-800/50 rounded-lg p-4">
                  ...
                  <div className="text-2xl font-bold text-green-400">
                    {gameHistory.length > 0 ? ((wins / (wins + losses)) * 100).toFixed(1) : '0.0'}%
                  </div>
                  <div className="text-slate-500 text-xs mt-1">{wins}-{losses} Record</div>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  ...
                  <div className="text-slate-500 text-xs mt-1">2025 Season</div>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <div className="text-slate-400 text-xs mb-1">Edge vs Breakeven</div>
                  <div className="text-2xl font-bold text-blue-400">
                    +{gameHistory.length > 0 ? (((wins / (wins + losses)) * 100) - 52.38).toFixed(1) : '0.0'}%
                  </div>
                  <div className="text-slate-500 text-xs mt-1">Above 52.4%</div>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <div className="text-slate-400 text-xs mb-1">Model Confidence</div>
